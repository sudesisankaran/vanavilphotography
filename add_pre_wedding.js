...
async function processImages() {
    ...
    // 2. Update pre-wedding.html
    if (fs.existsSync('pre-wedding.html')) {
        let content = fs.readFileSync('pre-wedding.html', 'utf8');
        
        // Fix Hero image (old path was images/pre-wedding-photos/photos/0L7A0844.webp)
        content = content.replace(/<img src="images\/pre-wedding-photos\/photos\/[^"]+" alt="BEFORE THE WEDDING, THERE WAS US" class="page-hero-bg" fetchpriority="high">/g, 
                                 '<img src="images/pre wedding/0L7A0844.webp" alt="BEFORE THE WEDDING, THERE WAS US" class="page-hero-bg" fetchpriority="high">');
        ...
        if (startIdx !== -1 && endIdx !== -1) {
            ...
            // Clean out old pre-wedding-photos links
            const imgRegex = /<img[^>]+src="images\/pre-wedding-photos\/photos\/[^"]+"[^>]*>/g;
            let match;
            let toRemove = [];
            ...
            for (const webpFile of webpFiles) {
                const encodedPath = `images/pre%20wedding/${encodeURIComponent(webpFile)}`;
                const rawPath = `images/pre wedding/${webpFile}`;
                
                if (!galleryHtml.includes(encodedPath) && !galleryHtml.includes(rawPath)) {
                    const imgTag = `\n                <img src="${encodedPath}" alt="Pre Wedding" class="fade-up" loading="lazy" decoding="async">`;
                    galleryHtml += imgTag;
                    added++;
                }
            }
            ...
            fs.writeFileSync('pre-wedding.html', content);
            console.log(`Cleaned old images and added ${added} new images to pre-wedding.html`);
        }
    }
    ...
    if (fs.existsSync('portfolio.html')) {
        ...
        for (const webpFile of webpFiles) {
            ...
            if (!content.includes(encodedPath) && !content.includes(rawPath)) {
                const entry = `\n                { img: '${encodedPath}', cat: 'pre-wedding', title: 'Pre Wedding', class: 'wide' },`;
