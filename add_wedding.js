...
async function processImages() {
    ...
    // 2. Update wedding-photography.html
    if (fs.existsSync('wedding-photography.html')) {
        let content = fs.readFileSync('wedding-photography.html', 'utf8');
        
        // Fix Hero image
        content = content.replace(/<img src="images\/wedding-2026[^"]+\/0M5A9624\.webp" alt="WEDDING PHOTOGRAPHY" class="page-hero-bg" fetchpriority="high">/g, 
                                 '<img src="images/wedding/0M5A9624.webp" alt="WEDDING PHOTOGRAPHY" class="page-hero-bg" fetchpriority="high">');
        ...
        if (startIdx !== -1 && endIdx !== -1) {
            ...
            const imgRegex = /<img[^>]+src="images\/wedding-2026[^"]+"[^>]*>/g;
            let match;
            let toRemove = [];
            ...
            for (const webpFile of webpFiles) {
                const encodedPath = `images/wedding/${encodeURIComponent(webpFile)}`;
                const rawPath = `images/wedding/${webpFile}`;
                
                if (!galleryHtml.includes(encodedPath) && !galleryHtml.includes(rawPath)) {
                    const imgTag = `\n                <img src="${encodedPath}" alt="Wedding Image" class="fade-up" loading="lazy" decoding="async">`;
                    galleryHtml += imgTag;
                    added++;
                }
            }
            ...
            fs.writeFileSync('wedding-photography.html', content);
            console.log(`Cleaned old images and added ${added} new images to wedding-photography.html`);
        }
    }
    ...
    if (fs.existsSync('portfolio.html')) {
        ...
        for (const webpFile of webpFiles) {
            ...
            if (!content.includes(encodedPath) && !content.includes(rawPath)) {
                const entry = `\n                { img: '${encodedPath}', cat: 'wedding', title: 'Wedding', class: 'wide' },`;
